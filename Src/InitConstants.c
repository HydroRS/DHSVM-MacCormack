		   ...
		   SOLARGEOMETRY * SolarGeo, TIMESTRUCT * Time)
{
  ...
  STRINIENTRY StrEnv[] = {
    ...
    {"OPTIONS", "KW FINITE DIFFERENCE", "", ""},    /* Added by Ling Zhang, 2016/8/28 */
	{"OPTIONS", "ITERATION TIME STEP", "", ""},     /* Added by Ling Zhang, 2016/8/28 */
  ...
  /** Modified by Ling Zhang, 2016/8/28 **/
  /* Determine overland flow routing method to use */
    if (strncmp(StrEnv[routing].VarStr, "KINEMATIC", 9) == 0)
    {
        Options->Routing = TRUE;
        if (strncmp(StrEnv[kw_finite_diff].VarStr, "EXPLICITLINEAR", 14) == 0)
            Options->KWFiniteDifference = EXPLICITLINEAR;
        else if (strncmp(StrEnv[kw_finite_diff].VarStr, "HUANGMACCORMACK", 15) == 0)
        {
            Options->KWFiniteDifference = HUANGMACCORMACK;
            if (!CopyFloat(&(Options->IterationTimeStep), StrEnv[iter_time_step].VarStr, 1))
                ReportError(StrEnv[iter_time_step].KeyName, 51);
        }
        else if (strncmp(StrEnv[kw_finite_diff].VarStr, "TSENGMACCORMACK", 15) == 0)
        {
            Options->KWFiniteDifference = TSENGMACCORMACK;
            if (!CopyFloat(&(Options->IterationTimeStep), StrEnv[iter_time_step].VarStr, 1))
                ReportError(StrEnv[iter_time_step].KeyName, 51);
        }
        else
            ReportError(StrEnv[kw_finite_diff].KeyName, 51);
    }
