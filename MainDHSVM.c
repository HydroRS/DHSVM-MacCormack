...
*****************************************************************************/
  ...
  /** ------------------------Added by LING ZHANG----------------------- **/
  LastTime = Time;
  DecreaseTime(&LastTime);

  if (Options.HasNetwork == TRUE) {
    InitChannelDump(&Options, &ChannelData, Dump.Path);
    /* Modified by LING ZHANG */
    ReadChannelState(Dump.InitStatePath, &(LastTime.Current), ChannelData.streams);
  }
  /** --------------------------End Add-------------------------------- **/
...
//		 SnowAlbedo, TopoMap, Network, &HydrographInfo, Hydrograph);
 ...
 /* Modified by LING ZHANG */
  InitModelState(&(LastTime.Current), &Map, &Options, PrecipMap, SnowMap, SoilMap,
		 Soil, SType, VegMap, Veg, VType, Dump.InitStatePath,
		 SnowAlbedo, TopoMap, Network, &HydrographInfo, Hydrograph);
...
/* Why there is another ExecDump, which is same as the previous time step, except the time
   added by LING ZHANG */
  ExecDump(&Map, &(Time.Current), &(Time.Start), &Options, &Dump, TopoMap,
